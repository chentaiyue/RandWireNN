...
# from Graph import *
# def bulid(input,c):
# 	graph_structure = ws(5,2,0.4)
#
# 	conv1 = tf.layers.conv2d(input,c,5,(2,2),'same',kernel_initializer=tf.random_normal_initializer(stddev=0.1))
# 	conv1 = tf.nn.relu(conv1)
# 	daglayer = DAGLayer(conv1,c,10,5,graph_structure)
#
# 	daglayer.set_shape([None,7,7,10])
# 	l = tf.layers.flatten(daglayer)
# 	l = tf.layers.Dense(10,tf.nn.relu)(l)
# 	logits = tf.nn.softmax(l,axis=-1)
# 	return logits
#
...
# 	logits = bulid(X,200)
# 	loss = tf.reduce_mean(-(y * tf.log(logits) + (1.0-y)*tf.log(1.0-logits)))
# 	update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
# 	with tf.control_dependencies(update_ops):
# 		train_op = tf.train.GradientDescentOptimizer(0.5).minimize(loss)
# 	accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(logits,1),tf.argmax(y,1)),tf.float32))
# 	mnist = input_data.read_data_sets(r'C:\Users\Secret\Desktop\point cloud\code\CapsNet-Tensorflow-master',
# 	                                  one_hot='true')
...
# 			acc,_,los = sess.run([accuracy,train_op,loss],{x:img,y:lab})
# 			print(acc,los)
#
# 		s = 0.0
# 		for i in range(100):
# 			img, lab = mnist.test.next_batch(100)
# 			acc = sess.run(accuracy, {x: img, y: lab})
# 			s += acc
#
# 		print(s/100.0)
